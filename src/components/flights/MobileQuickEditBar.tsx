...
const MobileQuickEditBar = ({
  ...
  cheapestPrice, currencySymbol, isLoading, formatDate,
  ...
  const closeOverlay = () => setIsExpanded(false);

  // Accessibility for expanded overlay
  const { containerRef, handleKeyDown } = useModalAccessibility(isExpanded, closeOverlay);

  const handleExpand = () => {
    setEditOrigin(origin); setEditOriginDisplay(origin);
    setEditDestination(destination); setEditDestDisplay(destination);
  ...
  const handleSearch = () => {
    ...
    const params = new URLSearchParams({
      origin: editOrigin.toUpperCase(), destination: editDestination.toUpperCase(),
      departureDate: format(editDepartDate, "yyyy-MM-dd"), passengers: String(editPassengers),
      adults: String(editPassengers), children: "0", infants: "0", cabinClass,
    });
    if (editReturnDate) params.append("returnDate", format(editReturnDate, "yyyy-MM-dd"));
    setIsExpanded(false);
    navigate(`/flights?${params.toString()}`);
  };

  const swapLocations = () => {
    const tmpCode = editOrigin, tmpDisplay = editOriginDisplay;
    setEditOrigin(editDestination); setEditOriginDisplay(editDestDisplay);
    setEditDestination(tmpCode); setEditDestDisplay(tmpDisplay);
  };

  return (
    <>
      {/* Collapsed bar */}
      <div className="bg-card border border-border shadow-lg rounded-2xl px-4 py-2.5 flex items-center gap-3 native-touch cursor-pointer" onClick={handleExpand}>
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-[11px] text-muted-foreground truncate">
            {formatDate(departureDate)}{returnDate && ` - ${formatDate(returnDate)}`} · {passengers} pax
          </p>
        </div>
          ...
          >
            ...
            >
              <div className="flex items-center justify-between">
                ...
                <button onClick={closeOverlay} className="w-9 h-9 rounded-full bg-secondary flex items-center justify-center native-touch">
                  <X className="h-4 w-4 text-foreground" />
                </button>
              </div>

              <div className="relative space-y-2">
                <button onClick={() => setFromPickerOpen(true)}
                  ...
                  <span className={cn("text-sm font-medium truncate", editOriginDisplay ? "text-foreground" : "text-muted-foreground")}>
                ...
                <button onClick={() => setToPickerOpen(true)}
                  className="w-full flex items-center gap-3 px-4 py-3 bg-secondary rounded-xl text-left native-press min-h-[48px]">
                  <Plane className="h-4 w-4 text-primary rotate-45 shrink-0" />
                  <span className={cn("text-sm font-medium truncate", editDestDisplay ? "text-foreground" : "text-muted-foreground")}>
                    {editDestDisplay || "To"}
                  </span>
                </button>
              </div>
              ...
              <div className="grid grid-cols-3 gap-2">
                <button onClick={() => setDepartPickerOpen(true)}
                  ...
                  <span className="text-xs font-semibold text-foreground">{editDepartDate ? format(editDepartDate, "d MMM") : "Select"}</span>
                </button>
                <button onClick={() => setReturnPickerOpen(true)}
                  className="flex flex-col items-start px-3 py-2.5 bg-secondary rounded-xl native-press min-h-[48px]">
                  <span className="text-[10px] font-medium text-muted-foreground">Return</span>
                  <span className="text-xs font-semibold text-foreground">{editReturnDate ? format(editReturnDate, "d MMM") : "-"}</span>
                </button>
                <div className="flex flex-col px-3 py-2.5 bg-secondary rounded-xl min-h-[48px]">
                  ...
                  <div className="flex items-center gap-2 mt-0.5">
                    <button onClick={() => setEditPassengers(Math.max(1, editPassengers - 1))} disabled={editPassengers <= 1}
                    ...
                    <button onClick={() => setEditPassengers(Math.min(9, editPassengers + 1))} disabled={editPassengers >= 9}
                      className="w-6 h-6 rounded-full border border-border flex items-center justify-center disabled:opacity-30 native-touch">
              ...
              <Button onClick={handleSearch} className="w-full h-12 text-sm font-semibold rounded-xl bg-accent hover:bg-accent/90 text-accent-foreground native-button">
                <Search className="h-4 w-4 mr-2" />Update Search
              </Button>
            </motion.div>
          </motion.div>
        )}
      ...
      <NativeLocationPicker isOpen={fromPickerOpen} onClose={() => setFromPickerOpen(false)}
        onSelect={(code: string, airport: Airport) => { setEditOrigin(code); setEditOriginDisplay(airport.city); }}
        title="Where from?" placeholder="Search airports or cities..." />
      <NativeLocationPicker isOpen={toPickerOpen} onClose={() => setToPickerOpen(false)}
        onSelect={(code: string, airport: Airport) => { setEditDestination(code); setEditDestDisplay(airport.city); }}
        title="Where to?" placeholder="Search airports or cities..." />
      <NativeDatePicker isOpen={departPickerOpen} onClose={() => setDepartPickerOpen(false)}
        onSelect={(date: Date) => { setEditDepartDate(date); if (editReturnDate && date > editReturnDate) setEditReturnDate(undefined); }}
        selected={editDepartDate} title="Select Departure Date" />
      <NativeDatePicker isOpen={returnPickerOpen} onClose={() => setReturnPickerOpen(false)}
        onSelect={(date: Date) => setEditReturnDate(date)} selected={editReturnDate} minDate={editDepartDate} title="Select Return Date" />
    </>
  );
};
