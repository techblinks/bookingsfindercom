...
import { Plane, Building2, Ticket, Search, ArrowRight, Shield, HeartHandshake, Library, ClipboardCheck, Calculator } from "lucide-react";
...
const QUICK_ACTIONS = [
  { label: "Flights", href: "/flights", icon: Plane, desc: "Explore flights" },
  { label: "Stays", href: "/hotels", icon: Building2, desc: "Find a stay" },
  { label: "Things", href: "/things-to-do", icon: Ticket, desc: "Find things to do" },
  { label: "Trip Cost", href: "/trip-cost", icon: Calculator, desc: "Plan trip cost" },
];
      ...
      <div className="min-h-screen flex flex-col bg-background pb-16">
        ...
        <main id="main-content" className="flex-1 px-4 pt-4">
          ...
          <Link
            ...
            className="flex items-center gap-3 w-full px-4 py-3.5 rounded-xl bg-muted/60 border border-border/50 text-left mb-6 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-primary"
          >
          ...
          {hasTrip && (
            <section className="mb-6 p-4 rounded-xl bg-primary/5 border border-primary/10" aria-labelledby="trip-context-heading">
              ...
              <div className="space-y-1 mb-4">
                ...
                {trip.dates?.departureDate && (
                  <p className="text-sm text-muted-foreground">
                    ...
                    {trip.travellers && ` · ${trip.travellers.adults + trip.travellers.children + trip.travellers.infants} traveller${(trip.travellers.adults + trip.travellers.children + trip.travellers.infants) !== 1 ? 's' : ''}`}
                  </p>
                )}
              </div>
              <div className="grid grid-cols-2 gap-2">
                {QUICK_ACTIONS.map(action => (
                  <Link
                    ...
                    className="flex items-center gap-2 px-3 py-2.5 rounded-lg bg-background border border-border/60 hover:border-primary/30 transition-colors text-sm font-medium focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-primary"
                  >
            ...
            <div className="grid grid-cols-2 gap-3">
              {QUICK_ACTIONS.map(action => (
                <Link
                  ...
                  to={action.href}
                  onClick={() => safeTrack(action.label, action.href)}
                  className="flex flex-col items-center justify-center gap-2 p-4 rounded-xl bg-card border border-border/50 hover:border-primary/30 hover:shadow-sm transition-all min-h-[88px] focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-primary"
                >
          ...
          {/* Trust / How it works (compact mobile) */}
          <section className="mb-8" aria-labelledby="trust-mobile-heading">
            ...
            <div className="flex flex-wrap gap-x-4 gap-y-1 mt-4">
              <Link to="/affiliate-disclosure" className="text-xs text-primary underline underline-offset-2">Affiliate disclosure</Link>
              <Link to="/privacy" className="text-xs text-primary underline underline-offset-2">Privacy policy</Link>
              <Link to="/terms" className="text-xs text-primary underline underline-offset-2">Terms of service</Link>
            </div>
          </section>

          {/* Tools */}
          <section className="mb-8" aria-labelledby="tools-mobile-heading">
            <h2 id="tools-mobile-heading" className="text-sm font-semibold text-foreground mb-3">Tools</h2>
            <div className="grid grid-cols-2 gap-2">
              <Link to="/trip-cost" onClick={() => safeTrack("trip_cost_tools", "/trip-cost")} className="flex items-center gap-2 px-3 py-2.5 rounded-lg bg-muted/40 border border-border/30 text-sm font-medium hover:border-primary/30 transition-colors focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-primary">
