...
describe("Flights V1 Mobile — sort", () => {
  it("mobile results show Cheapest/Fastest/Fewest stops segmented control (no 'Best' — no provider-defined ranking exists for cached data)", () => {
    hoisted.isMobile = true;
    renderFlightResults(RESULTS_ROUTE);
    expect(screen.getByText("Cheapest")).toBeTruthy();
    expect(screen.getByText("Fastest")).toBeTruthy();
    expect(screen.getByText("Fewest stops")).toBeTruthy();
    expect(screen.queryByText("Best")).toBeNull();
  });

  it("Cheapest sort has aria-checked=true by default", () => {
    hoisted.isMobile = true;
    renderFlightResults(RESULTS_ROUTE);
    const cheapestBtn = screen.getByRole("radio", { name: "Cheapest" });
    expect(cheapestBtn.getAttribute("aria-checked")).toBe("true");
  });
