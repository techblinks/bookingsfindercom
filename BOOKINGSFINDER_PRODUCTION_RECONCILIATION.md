...
**ORIGINAL INVESTIGATION BASE** (Phases 1–7 of this document — the forensic evidence-gathering: authority proof, migration-gap discovery, evidence matrices): `main` at `2c73d87be6cdded72e705b12f89f816e1c8d5e2b`, before any change in this workstream. That commit is **not** the current state of anything — it is the baseline the investigation started from.

**CURRENT PR HEAD** (this branch, as actually proposed for merge): see PR #65, branch `fix/bf0r4-production-reconciliation`. The exact head SHA is authoritative in the PR itself, not restated here as a fixed value, since this document is amended in place across multiple commits on that branch — check `git rev-parse HEAD` on the branch, or the PR page, for the live value.

**Base**: current `main` after merged PR #64 (BF-0R-3), #63 (BF-0R-2), #62 (T4A-P2R-F1).

**Nature of this workstream, precisely**: this document mixes two different kinds of action, and they must not be conflated —

1. **Production database operations already executed** against the live authoritative project `pjehrnhmjrxrlrhuhqgf`, under explicit, staged, per-step authorization from you: a full backup, then two migrations applied (§7.9). **This DID mutate the remote database** — the earlier claim in this document that "no remote database mutation occurred" was accurate only for the phases before §7.9 and is **superseded** by it. No further remote mutation has occurred since.
2. **Repository code changes**, committed to this PR branch, currently **not merged and not deployed**: the run-optimizer authentication and atomic-quota fix, its tests, and this document. The earlier claim "no repository files were changed" was accurate only for the original forensic phase and is **superseded** by every commit on this PR branch.

No Edge Function has been deployed. No secret has been changed. No merge has occurred.

---

## ROUND 4 UPDATE (2026-08-23) — main has advanced; branch reconciled; three more defects found and fixed

**This section reflects CURRENT truth as of round 4. It supersedes stale SHA references elsewhere in this document (§2, §7) for the purpose of "what is main today" — those sections are left as an accurate historical record of what was true when they were written, not edited to pretend otherwise.**

- **Current `origin/main` SHA: `6d381d7137ef959db165dfa102ddce7dcef838de`.** Since this document's original investigation baseline (`2c73d87`), `main` has advanced through **four merged, unrelated PRs, all already production-live and independent of PR #65**:
  - PR #66 — **BF-0R-5**: Data API RLS hardening (`60ba8f0`) — production database reconciliation and RLS hardening, already applied and live.
  - PR #67 — **BF-0R-7**: Travelpayouts `prices_for_dates` correctness, exact-date matching, HTML-embed ad-safety fix, cabin-class support (`78dcac1`).
  - PR #68 — **BF-0R-7.1**: Business-search cached-fare-leak cleanup (`13314dd`).
  - PR #69 — **BF-0R-7.2**: flight empty-state truth cleanup (`6d381d7`).
  - **None of these touch any file PR #65 touches** — confirmed by direct diff comparison (`git diff --name-only` on each side against their merge-base): zero file overlap. `origin/main` was merged into `fix/bf0r4-production-reconciliation` with **zero conflicts** as a result — not because of manual resolution, but because the two changesets are genuinely disjoint.
- **This PR branch has been reconciled with current `main`** via a plain `git merge origin/main` (no rebase, no force push) — see the PR's merge commit. `supabase/config.toml`, the shared Travelpayouts helpers, generated Supabase types, and every optimizer file were individually confirmed to have no cross-branch conflict.
- **Round 4 code review found and fixed three additional defects**, all repository-only, none deployed:
  1. **`get-subscription-status` was a second, uncoordinated writer of optimizer quota state** — see new §9.1.2. Fixed: it is now read-only with respect to `monthly_optimizer_uses`/`last_optimizer_reset`.
  2. **`run-optimizer`'s atomic claim path conflated a genuine database error with a legitimate optimistic-concurrency conflict** — see §9.1.2. Fixed: the two are now distinguished, and a genuine DB error fails closed immediately rather than retrying.
  3. **The frontend's `TripOptimizer.handleSubmit` had no synchronous guard before its first `await`**, and `useOptimizer.runOptimizer` had no protection against a stale response/error overwriting a newer attempt's state — see §9.1.2. Both fixed.
- **`run-optimizer` and `get-subscription-status` remain NOT deployed** — round 4's fixes are repository-only, pending your review, merge, and a separate, explicitly-authorized deployment, exactly as rounds 1–3 were.
- **No migration, no database write, no secret change, no deployment occurred in round 4.**
...
3. **`run-optimizer`**: confirmed not deployed. Two rounds of external PR review each found a real P0 in the round-1 fix, both now addressed **in the repository only** (not deployed, not merged): (a) anonymous callers could bypass the quota entirely and trigger unlimited real provider calls — closed by mandatory authentication (§9.1); (b) even once authenticated, the quota check was non-atomic and fail-open — two concurrent Free requests could both pass, a failed profile read defaulted to "free, 0 uses," and a failed request-insert still let the provider call through — closed by an atomic Postgres compare-and-set claim/refund (§9.1.1). Do not call this "repaired" in the production sense: it is repaired **in this PR's diff**, pending your review, merge, and a separate, explicitly-authorized deployment. This is a documented, planned **BF-0 final-gate blocker**, not an unaddressed reconciliation gap.
...
- ~~**Base `main` SHA** (unchanged since the investigation began — no one else has merged to `main` during this workstream): `2c73d87be6cdded72e705b12f89f816e1c8d5e2b`~~ — **SUPERSEDED as of round 4 (see the ROUND 4 UPDATE section near the top of this document).** That claim was accurate through round 3; it is no longer true. Current `origin/main` is `6d381d7137ef959db165dfa102ddce7dcef838de`, after four merged PRs (#66–#69) unrelated to this branch. This branch has since been merged with current `main` (plain `git merge`, no rebase, zero conflicts, zero file overlap) — see the round 4 update for detail. `2c73d87` remains accurate as the SHA the *original forensic investigation* (§1–§7) was performed against; it is not restated as current.
- **What the commits on this branch contain, in order**: (1) this reconciliation document itself; (2) `run-optimizer`'s mandatory-authentication fix plus a frontend sign-in gate, closing an anonymous quota-bypass P0; (3) `run-optimizer`'s atomic Free-quota claim/refund, closing a concurrent-request-race and fail-open-lookup P0 found in review of (2); (4) a refund-token fix closing an ABA/month-boundary race found in review of (3); (5) round 4 — a merge of current `main`, plus `get-subscription-status`'s read-only fix, `run-optimizer`'s claim-error/CAS-conflict distinction, and the frontend single-flight/stale-response fix.
- **Status, explicitly**: all of the above are **committed to this PR branch and pushed to GitHub — not merged into `main`, not deployed anywhere, no secret changed, no production database further mutated** beyond the migrations already recorded in §7.9 (which were explicit, separately-authorized production operations, not part of this PR's diff) and beyond the four PRs (#66–#69) merged to `main` independently of this branch and now incorporated via round 4's merge.
...
|---|---|---|---|---|---|---|---|
| `run-optimizer` | BF-0R-2 (`5db66f3`) rewrote fabricated travel-intelligence logic; BF-0R-4 rounds 1–3 closed the anonymous quota-bypass and atomic-claim defects (§9.1/§9.1.1); **round 4 additionally distinguished claim-error-vs-conflict semantics — see §9.1.2B** | Customer feature (not admin-gated) — **requires any authenticated user**, defense-in-depth via `auth-quota-core.ts`, same convention as `_shared/admin-auth.ts` | `true` (unchanged since round 1, confirmed intact after round 4's main-merge) | **NOT DEPLOYED** — repaired locally across four review rounds, not yet merged or deployed | Not live-probed (explicitly forbidden); local fix implemented and test-verified | Yes | Not yet — pending your review/merge, then deploy under §13 |
| `get-subscription-status` | Pre-existing; **round 4 (§9.1.2A) fixed a second-writer race against `run-optimizer`'s quota state** | Function-level auth (Bearer token + `supabase.auth.getUser`), not platform-gated | `false` (function performs its own auth check; unrelated to the fix) | **NOT DEPLOYED** — repaired locally this round, not yet merged or deployed | Not live-probed; local fix implemented and test-verified this round | Yes | Not yet — pending your review/merge, then deploy under §13, alongside `run-optimizer` |
...
**Fixed in this PR's diff** (not deployed, not merged):
...
- `supabase/functions/run-optimizer/index.ts`: auth is now resolved and required before anything else runs; a missing/invalid token returns 401 with no side effects.
- Frontend: new `src/components/optimizer/OptimizerAuthGate.tsx` (reuses `src/pages/Account.tsx`'s exact `supabase.auth.signInWithPassword`/`signUp` calls — no new auth system) wired into `src/pages/TripOptimizer.tsx`: a signed-out submit is held and auto-continued once a genuine session appears via `onAuthStateChange`, instead of firing a doomed anonymous request. Explicitly handles the case where `signUp` returns no session (email confirmation required) without ever invoking the optimizer itself, and cannot double-fire the held request from repeated auth events (proven by test).
...
### 9.1.1 SECOND P0 (PR #65 review, round 2) — quota check was non-atomic and fail-open, now fixed in this PR

A second external review of the round-1 diff found the free-quota accounting itself was not safe under concurrency or partial failure, even once anonymous access was closed:

1. **Concurrent-request race**: `read monthlyUses → decide < FREE_LIMIT → insert optimizer_requests → call Travelpayouts → only then increment` is five separate steps, not one. Two authenticated Free requests firing at nearly the same time could both read `monthlyUses = 0`, both pass the check, and both reach the real provider before either incremented — "one Free optimization per month" was not actually enforced. Frontend duplicate-submit protection is a UX nicety, not a security boundary: a signed-in caller can invoke the Edge Function directly.
2. **Fail-open profile lookup**: a `user_profiles` query error, or a missing row, was silently treated as `plan="free", monthlyUses=0` and the provider call proceeded anyway.
3. **Fail-open request-insert**: a failed `optimizer_requests` insert was logged and ignored, and the provider was called regardless — a real provider call could happen with no durable record and no quota accounting.

**A. Atomic quota design.** `supabase/functions/run-optimizer/optimizer-orchestrator.ts` (new) is a Deno-global-free orchestration module — every DB/provider operation is an injected dependency (`OptimizerDbPort`), so its control flow is exercised by REAL mocked orchestration tests, not source-position regex. For a Free (or inactive-Pro, downgraded to the same contract) user:
   - The profile is read once. Any read error, or an absent row, **fails closed** immediately (503) — no claim, no request write, no provider call.
   - If allowed, a slot is **atomically claimed** via a single Postgres compare-and-set `UPDATE user_profiles SET monthly_optimizer_uses = … WHERE user_id = $1 AND monthly_optimizer_uses = $2 AND last_optimizer_reset = $3` — the `WHERE` matches the *exact* row values just read. The month-reset transition and the increment are the **same statement** (whichever applies), so a fresh-month race can't let two requests both reset-and-claim.
   - Only **after** the slot is claimed is `optimizer_requests` written. A failed insert **refunds** the claim (a symmetric compare-and-set decrement) and returns 503 — the provider is never called.
   - The provider is then called. A non-`"ok"` outcome (`provider_error`, `provider_unavailable`, `no_results`, `unusable_results`) **refunds** the claim — insufficient live data must never consume the Free allowance, matching the pre-existing product rule, now made durable rather than merely "increment only happens after a stored result."
   - A genuine `"ok"` outcome keeps the claim (no refund) and persists the result — the slot is durably consumed by exactly one successful optimization.
   - **Active Pro** (`subscriptions.status = "active"`) never enters the claim/refund cycle at all — unchanged, unlimited, as before.

**B. Why two concurrent requests cannot both pass.** Postgres re-evaluates an `UPDATE`'s `WHERE` clause against the row's *current committed value* after acquiring its row lock (standard READ COMMITTED semantics) — not against a snapshot taken before the lock. So if request A's `UPDATE` commits first (row now has the new `monthly_optimizer_uses`/`last_optimizer_reset`), request B's `UPDATE`, which is still matching the *old* values it read, finds zero matching rows and its `.select().maybeSingle()` returns `{data: null}` — a clean, deterministic "you lost the race" signal, not a partial or corrupted update. The orchestrator then re-reads and recomputes once (bounded single retry): if the fresh state shows the allowance is now exhausted, it rejects with the paywall; it never loops indefinitely. This is proven directly — not just argued — by a real test that fires two `handleOptimizerRequest` calls via `Promise.all` against one shared fake store whose `claimFreeSlot` implements the same compare-and-set contract, and asserts the provider was called exactly once and the store ends at exactly one consumed slot.

**C. Missing-profile / query-error behavior.** Both **fail closed**: a thrown/rejected profile read, or a `null` result (no row), returns a 503 service error before any claim, request write, or provider call — proven by dedicated orchestration tests. (A trigger normally creates a `user_profiles` row on signup, but this fix does not rely on that trigger always having run — it fails closed regardless of why the read came back empty or errored.)

**D. `optimizer_requests` insert-failure behavior.** If the slot was already claimed (Free/inactive-Pro) and the insert then fails, the claim is refunded via the same compare-and-set decrement pattern, and a 503 is returned — the provider is never called. Proven by test, including that the refund actually lands (the store's `monthly_optimizer_uses` returns to its pre-claim value).

**E. No-data refund behavior.** All four insufficient-data reasons refund the claim identically — proven by a parameterized test over all four reasons — and additionally prove no `optimizer_results` row is ever written for a non-`"ok"` outcome (the BF-0R-2 "genuine-answer-only persistence" rule, now proven at the orchestration level rather than only by source-text regex).
...
### 9.1.2 ROUND 4 (PR #65 review) — three more defects found and fixed, plus a documented residual-risk classification

**A. `get-subscription-status` was a second, uncoordinated writer of optimizer quota state.**

`supabase/functions/get-subscription-status/index.ts` — a separate, previously-unaudited-in-this-document Edge Function called by the frontend to display remaining-optimizations UI — performed its **own** unconditional `UPDATE user_profiles SET monthly_optimizer_uses = 0, last_optimizer_reset = now()` whenever it locally decided a new calendar month had started, using **local server time**, not UTC. This has two problems: (1) it is a second writer to the exact durable row `run-optimizer`'s compare-and-set claim path depends on being the *sole* writer of — a status read racing a concurrent `run-optimizer` claim could silently move `last_optimizer_reset` out from under the claim's CAS guard, corrupting the "one Free optimization per month" contract; (2) using local time (not UTC) meant a status read and a concurrent claim could, right around a month boundary, disagree about whether a reset was even due.

**Fixed in this PR's diff** (not deployed): new pure module `supabase/functions/get-subscription-status/quota-view.ts` (`computeNeedsReset`, `computeEffectiveQuotaView`) — UTC-based, matching `optimizer-orchestrator.ts`'s existing `computeNeedsReset` semantics exactly. `index.ts` now calls this module to compute a **virtual, display-only** effective usage figure and **no longer writes to `user_profiles` at all**. `run-optimizer` remains the sole writer of optimizer quota fields. Proven by dedicated tests, including one that models a status read landing between `run-optimizer`'s profile read and its claim CAS and asserts the claim's expected-value guard is unaffected.

**B. The atomic claim path conflated a genuine database error with a legitimate optimistic-concurrency conflict.**

`claimFreeSlot`'s real implementation (`run-optimizer/index.ts`) returned the same `{claimed: false}` shape whether the compare-and-set genuinely lost a race (zero rows matched, no query error — expected and safe to retry once against fresh state) or the query itself failed (a real database/PostgREST error). The orchestrator's single-retry logic then treated both identically: re-read the profile and attempt a second claim. This never let a failed claim reach the provider (already correct), but it meant a real database outage would trigger a second, likely-also-failing database round-trip before finally surfacing an error, and — in principle — a persistent outage landing on a fresh-read state that happened to show the allowance exhausted could be reported as a false paywall rather than an honest "something is broken" message.

**Fixed in this PR's diff** (not deployed): `ClaimSlotResult`'s `{claimed: false}` case now carries `reason: "conflict" | "error"`. The orchestrator fails closed **immediately** (503, no retry) on `reason: "error"`, and only takes the existing re-read-and-retry path on `reason: "conflict"`. Proven by tests: a DB error on the first claim attempt makes exactly one claim call total (no retry) and returns 503; a persistent DB error across both attempts still returns 503, never a paywall; a genuine CAS conflict still retries and can still succeed, unchanged.

**C. Frontend rapid-submit race: no synchronous lock before the first `await`, and no protection against a stale response overwriting a newer attempt.**

`TripOptimizer.tsx`'s `handleSubmit` began with `await supabase.auth.getSession()` with no synchronous guard in place beforehand — two rapid clicks (or any duplicate synthetic submit) could both enter `handleSubmit` and reach `getSession()`/`runOptimizer` concurrently before React had re-rendered a disabled/loading button. Separately, `useOptimizer.runOptimizer` used plain `setState` calls with no attempt tracking: if two calls ever did overlap, whichever network response resolved **last** — not whichever was started most recently — won the final `isLoading`/`error`/`paywallError` state, a classic stale-response-overwrites-newer-attempt race.

**Fixed in this PR's diff** (not deployed): `TripOptimizer.tsx` gained a `submitInFlightRef` set synchronously to `true` at the top of `handleSubmit`, **before** the first `await`; a duplicate call arriving while it is held returns immediately, and it is released in a `finally` covering every exit path (normal completion, the signed-out early return, and a thrown error), so Reset/Cancel/auth transitions never find it stuck. `useOptimizer.ts` gained a `currentAttemptRef`-based attempt id: each `runOptimizer` call captures its own id at start, and only commits `setIsLoading`/`setError`/`setPaywallError`/its return value if it is still the most recent attempt when its network call resolves — a stale response or error from a superseded attempt is discarded rather than overwriting newer state. Proven by tests firing two synchronous clicks before the mocked session lookup resolves (exactly one `runOptimizer` invocation results), and by tests resolving two overlapping `runOptimizer` calls out of order (the stale one's result/error never reaches state).

**D. Process-death / idempotency — explicitly classified, not solved here (by design).**

Three residual reliability risks were assessed and are documented as **P1 follow-ups (§15), not BF-0 final-gate blockers**, because none creates a security or cost-abuse path — each one is neutral-to-punitive toward the traveller (a lost quota slot, not a bypassed one), never permissive:

1. **Process/function death after a quota slot is claimed but before the provider result is known.** The claimed slot has no refund path if the Edge Function process itself dies mid-request (as opposed to returning a typed failure, which IS refunded — see §9.1.1E). Low-probability; the cost is a traveller support ticket, not an exploitable bypass.
2. **Response lost after successful provider work completes.** Same category — the slot was legitimately consumed for a genuine result the traveller never received. Not an abuse path.
3. **No request-level idempotency key.** A traveller who manually resubmits after a perceived failure creates a genuinely new attempt, which is correct current behavior for an explicit new submission — the existing refund-on-non-`"ok"`-outcome logic (§9.1.1) already prevents a *failed* attempt from consuming a slot, which covers the common case. A durable lease/idempotency-key architecture would close the remaining process-death edge case but was judged unnecessary scope for this PR — it does not block merge.
...
5. `run-optimizer` / `generate-route-page` / `generate-seo-content` not deployed — **reclassified, not a reconciliation-phase P0**: `run-optimizer` has a documented plan plus two now-repaired-in-this-PR defects (§9.1, §9.1.1); the two AI functions are a **deliberate withhold** (§9.2), not an unaddressed gap. These are **BF-0 final-gate** items (below), not BF-0R-4 blockers.
...
8. **`run-optimizer` refund ABA/month-boundary race** — a third round of review found the round-2 refund still matched on the quota counter alone, letting a stale refund (from a request still in flight across a UTC month boundary) silently zero out an unrelated later request's fresh claim (§9.1.1). Fixed in this PR's diff via a durable refund token matching both `monthly_optimizer_uses` and `last_optimizer_reset`; proven directly by a test that holds one request unresolved across a simulated month boundary while a second completes a fresh claim. Tested with real mocked orchestration tests proving the race cannot be won twice, including this month-boundary case (§9.1.1).
9. **`get-subscription-status` second-writer race against `run-optimizer`'s quota state** — round 4 found this previously-unaudited function performed its own unconditional, local-time-based reset write to the same row `run-optimizer`'s CAS claim depends on being the sole writer of (§9.1.2A). Fixed in this PR's diff: now strictly read-only re: quota, UTC-based, matching `run-optimizer`'s reset semantics exactly.
10. **`run-optimizer` claim-error/CAS-conflict conflation** — round 4 found a genuine database error and a legitimate optimistic-concurrency loss returned the same `{claimed: false}` shape, so a real DB outage would be retried once before finally failing, rather than failing closed immediately (§9.1.2B). Fixed in this PR's diff: the two are now distinguished; a genuine error fails closed on the first attempt, never retried.
11. **Frontend rapid-submit / stale-response race** — round 4 found `TripOptimizer.handleSubmit` had no synchronous lock before its first `await`, and `useOptimizer.runOptimizer` had no protection against a stale response overwriting a newer attempt's state (§9.1.2C). Fixed in this PR's diff: a synchronous ref-based single-flight guard, and an attempt-id guard on every state commit.

**Items 6 through 11 are all committed to PR #65, not merged, not deployed.** All six must be reviewed, merged, and safely deployed (`run-optimizer` **and** `get-subscription-status`) before the Optimizer can go live.

**P1 — should repair before BF-1**
12. **`publish-scheduled-pages`** — reclassified from P2 to P1 in a prior step. `verify_jwt=false`, **no in-function authorization check of any kind**, constructs its Supabase client with `SUPABASE_SERVICE_ROLE_KEY`, and performs a privileged `UPDATE country_landing_pages SET is_published=true` for any row whose `scheduled_publish_at` has passed. Confirmed: **no active exposure** (not deployed) and **no existing scheduler contract** to build against (§9.3) — a future design decision is required before it can ship. Not changed in this PR.
13. `www.bookingsfinder.com` returning 522 — likely a DNS/proxy gap, not urgent but should be resolved or intentionally documented as unsupported.
14. **Optimizer process-death / idempotency (round 4, §9.1.2D)** — three residual reliability risks, explicitly classified rather than solved in this PR: (a) a claimed quota slot has no refund path if the Edge Function process dies between the claim and the provider result (as opposed to returning a typed failure, which IS refunded); (b) a successful provider response can be lost after the slot is legitimately consumed; (c) there is no request-level idempotency key, so a manual resubmit is a genuinely new attempt. None creates a security or cost-abuse path — each is neutral-to-punitive toward the traveller, never permissive — so none blocks merge. A durable lease/idempotency-key architecture would close the remaining process-death edge case but was judged unnecessary scope for this PR.

**P2 — cleanup/later**
15. Dead fabricated-marketing component files (`TopDeals.tsx`, `WhyBookWithUs.tsx`, `AirlineOffers.tsx`, `DynamicDeals.tsx`, `PopularDestinationsCards.tsx`) remain in the repo unreferenced — candidates for deletion once confirmed truly unused.
16. No public, non-secret build-metadata (git SHA / build timestamp) endpoint exists, which would materially simplify future reconciliation phases like this one.

---

## 16. BF-0 Final-Gate Readiness

**Not ready — but the gap is now narrow, specific, and addressed in a PR pending your review.** The trust repairs (BF-0R-2, BF-0R-3, T4A-P2R-F1) — and, as of the round 4 update above, BF-0R-5 and BF-0R-7/7.1/7.2 as well — are correctly merged into `main` and, at the frontend/Worker layer, are live in production with high confidence. Authority is proven. The database layer is fully reconciled (30/30, privilege defect fixed and verified). **The sole remaining BF-0 final-gate blocker is the customer-facing Optimizer**: the anonymous-quota-bypass fix (§9.1), the atomic-quota-integrity fix (§9.1.1), and round 4's three additional fixes (§9.1.2 — `get-subscription-status`'s second-writer race, claim-error/CAS-conflict conflation, and the frontend rapid-submit race) are all implemented and test-verified in PR #65, but **not yet merged or deployed**. `generate-route-page`/`generate-seo-content` are a deliberate, documented withhold (§9.2) — not a final-gate blocker, since BF-0 does not require an AI-provider decision to have already been made. `publish-scheduled-pages` remains an explicitly deferred, undeployed P1 design item. The process-death/idempotency risks (§9.1.2D) are explicitly classified P1 follow-ups, not final-gate blockers.

## 17. Recommendation for BF-1

**Do not begin BF-1 yet — but BF-0R-4's own scope is complete.** Recommended order: (1) review and merge PR #65 (the `run-optimizer` auth + atomic-quota fix, §9.1/§9.1.1, plus round 4's `get-subscription-status` read-only fix, claim-error semantics, and frontend single-flight fix, §9.1.2); (2) as a separate, explicitly-authorized controlled step, deploy **both** `run-optimizer` and `get-subscription-status` per §13, with the same backup-first/verify-after discipline used for the migrations in this workstream; (3) smoke-test that a signed-out submit is correctly gated, that a signed-in Free/Pro user gets the intended allowance, specifically that two rapid successive submissions from the same account cannot both succeed, and that the displayed remaining-optimizations count after a fresh claim matches what `run-optimizer` actually stored (proving `get-subscription-status`'s read no longer races the claim), in production; (4) re-run this reconciliation's live checks (§3/§5) to confirm post-deployment state; (5) only then close BF-0's final gate. `generate-route-page`/`generate-seo-content` and `publish-scheduled-pages` remain deliberately deferred (§9.2, §9.3) and do not block this sequence — they are separate, future, deliberate decisions. Begin BF-1 only after BF-0's final gate (step 5) closes.
...
**BF-0R-4 PASS — scoped explicitly to production/Supabase authority and database reconciliation.** Authority is proven (`pjehrnhmjrxrlrhuhqgf`), production database state is reconciled (30/30 migrations, confirmed privilege defect closed), and every currently-undeployed function has an honest, specific status (planned-and-in-review, or deliberately withheld) rather than an unaddressed gap. This verdict does **not** certify that `run-optimizer`'s (or `get-subscription-status`'s) code is production-ready to deploy as-is, nor that any Edge Function deployment has occurred.

**BF-0 FINAL GATE: BLOCKED** — pending review and merge of PR #65 (`run-optimizer` mandatory authentication §9.1, atomic Free-quota claim/refund §9.1.1, **and round 4's `get-subscription-status` read-only fix / claim-error semantics / frontend single-flight fix, §9.1.2**) and its safe, explicitly-authorized deployment of **both** `run-optimizer` and `get-subscription-status`.

---

## ROUND 4 STATUS SUMMARY (2026-08-23)

- Production DB reconciliation: **already complete** (§7.9) — unaffected by this round, no new database action taken.
- BF-0R-5 (Data API/RLS hardening): **already merged to `main` and production-live** — PR #66, independent of PR #65.
- Flights BF-0R-7 / 7.1 / 7.2: **already merged to `main` and production-live** — PRs #67/#68/#69, independent of PR #65.
- `run-optimizer`: **repository-only fixes across four review rounds; still NOT deployed.**
- `get-subscription-status`: **repository-only fix this round; still NOT deployed.**
- This branch (`fix/bf0r4-production-reconciliation`) has been merged with current `main` (`6d381d7137ef959db165dfa102ddce7dcef838de`), zero conflicts, zero file overlap.
- Round 4 focused/optimizer test count: **102 passing** (run-optimizer's three suites, `get-subscription-status`'s new suite, and the three frontend/hook Optimizer suites, combined).
- Known P1 residual risks: process/function death after claim (§9.1.2D-1), response loss after success (§9.1.2D-2), no request-level idempotency key (§9.1.2D-3) — none blocks merge; none is a security/cost-abuse path.
- **Do not deploy `run-optimizer` or `get-subscription-status` on the strength of this document alone** — a separate, explicit deployment authorization remains required, exactly as in rounds 1–3.
